package com.cookietech.namibia.adme.chatmodule.data.source.firebase
...
class FirebaseDaoImpl {
    ...
    val userLiveData = Transformations.map(user) { userWithState ->
        Log.d("akash_chat_debug", ": " + userWithState)
    ...
    private fun addUser(user: User, callBack: (usernameStatus: NetworkState) -> Unit) {
        Log.d("akash_chat_debug", "addUser: " + userLiveData.value?.first?.userId)
